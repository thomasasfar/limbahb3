        ...
        <div class="table-responsive">
            <table id="multi-filter-select" class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <form action="/terima_pengantaran/{{$aktivitas_limbah->id}}" method="POST"
                        ...
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>
                                <select name="id_aktivitasmasuk[]" class="form-select" readonly>
                                    @foreach ($klasifikasi_limbah as $l)
                                    <option value="{{ $lmbh->id }}" {{ $l->
                                        id ==
                            ...
                            <td>
                                <select name="kode_limbah[]" class="form-select" disabled>
                                    @foreach ($klasifikasi_limbah as $l)
                                    <option value="{{ $l->id }}" {{ $l->
                                        id ==
                                        $lmbh->id_klasifikasilimbah ?
                            ...
                            <td>
                                <input type="text" name="keterangan[]" class="form-control"
                                    value="{{$lmbh->keterangan}}" placeholder="Keterangan (Opsional)">
                            </td>
                        </tr>
                        @endforeach
                </tbody>
        ...
        <table>
            ...
            <tr style="text-align: center;font-weight:bold">
                <td colspan="3" style="text-align:center;">
                    <br>
                    <input class="form-control" type="text" name="menerima" placeholder="Masukan nama personil penerima">
                    <button type="submit" class="btn btn-success mx-3">Terima
                        Pengantaran</button><br>
